...
interface Order {
  ...
  items: { name: string; qty: number; price: number; selectedSize?: string; customSize?: { bust?: string; waist?: string; hip?: string } }[];
  total_amount: number;
                  ...
                  <div>
                    ...
                    {(o.items || []).map((item, i) => (
                      ...
                      >
                        ...
                        {item.selectedSize === "Custom" && item.customSize && (
                          <div style={{ display: "flex", gap: 12, marginTop: 5 }}>
                            {item.customSize.bust && (
                              <span style={{ fontSize: 11, background: "#fdf8f5", border: "1px solid #e8ddd8", borderRadius: 6, padding: "2px 8px", color: "#7A5555" }}>
                                Bust: <strong>{item.customSize.bust}"</strong>
                              </span>
                            )}
                            {item.customSize.waist && (
                              <span style={{ fontSize: 11, background: "#fdf8f5", border: "1px solid #e8ddd8", borderRadius: 6, padding: "2px 8px", color: "#7A5555" }}>
                                Waist: <strong>{item.customSize.waist}"</strong>
                              </span>
                            )}
                            {item.customSize.hip && (
                              <span style={{ fontSize: 11, background: "#fdf8f5", border: "1px solid #e8ddd8", borderRadius: 6, padding: "2px 8px", color: "#7A5555" }}>
                                Hip: <strong>{item.customSize.hip}"</strong>
                              </span>
                            )}
                          </div>
                        )}
